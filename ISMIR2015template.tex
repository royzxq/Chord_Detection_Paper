...
\begin{figure}
 ...
 \caption{The overview of our system}\footnote{change font size in this figure so that all boxes can have the same horizontal width AND that the font size matches the font size in other graphs}
 \label{fig:overview}
\end{figure}
...
It can observed that the network trained with filtered inputs slightly outperforms the network without pre-processing; splicing the filtered input with the extension filter outputs increases the results significantly ($p=???$)\footnote{compute significance}.
...
Bottleneck & Spliced Filters & 0.936 & \textbf{0.919} \\ \bottomrule
\end{tabular*}
\caption{Chord detection performance for different architectures and pre-processing steps}
\label{tab:arch}
\end{table}

The results show that for the two pre-processing scenarios tested, the bottleneck architecture gives significantly better results ($p=???$)\footnote{compute significance} on the test set (WSCR). This is not true for the training set (Training WSCR), for which the common architecture achieves results in the same range or better than the bottleneck architecture. The difference between the results on the training set and the test set are thus much larger for the common architecture than for the bottleneck architecture.
...
As can be seen from the table, the performance for multi-label results is clearly worse than the result for single-label training.


\section{Discussion}
\subsection{Learning strategy \& classifiers}
The results listed in Table~\ref{tab:dbn-dnn} are unambiguous and unsurprising: the combination of unsupervised and supervised training significantly outperforms learning strategies that use either supervised or unsupervised training.\footnote{Why is supervised training so much worse than dbn-dnn? Explain!} It is also unsurprising that the HMM with Viterbi decoding outperforms the SVM; using HMMs with a model for transition probabilities is an appropriate approach to chord detection as it models the dynamic properties of chord progressions, which cannot be done with non-dynamic classifiers such as SVMs. One noteworthy result is that the SVM does not improve the WCSR compared to the direct (Argmax) output of the network. Apparently, the SVM is not able to improve separability of the learned output features.

\subsection{Pre-processing}
Although we did not use a CNN, the use of pre-filtering and splicing of the input representation somewhat emulates a simple CNN. While a low-pass filter by itself increases the classification performance only slightly, the splicing of the outputs of different filters has a large effect on the classification rate, as shown in Table~\ref{tab:filter}.
...
\begin{inparaenum}[(i)]
    ...
    \item   the issue that not all pitches always sound simultaneously in a chord (or might be missing altogether) might have larger impact on the multi-label training than on the single-label training.\footnote{talk somewhere here about the other paper that somehow used pitch chroma.}    
\end{inparaenum}


\subsection{Overall results}
It it always challenging to compare previous results because of the varying evaluation methodologies, metrics, and datasets. It seem that the results of Cho and Bello, who reported a performance of about 80\%, were computed with a comparable dataset. In order to provide a baseline result to put results into perspective, we present the results of Chordino \cite{}, computed on our dataset. It should be pointed out that this comparison is unfair as Chordino is able to detect as many as 120 chords, compared to our 24. The chordino results are mapped to triads the same way as the ground truth annotations. The results are shown in Table~\ref{tab: final}.\footnote{You write this: ``We present the mean accuracy of 3 fold cross-validation obtained on our 311-piece dataset at the major/minor level (total 25 classes).'' Shouldn't this go to methodology???}
...
Chordino & 0.625       \\
best configuration &  0.919   \\ \bottomrule
\end{tabular*}
